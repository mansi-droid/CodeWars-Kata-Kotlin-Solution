...
class SubtractSumTest {
    @Test
    fun testSubtractSum() {
        Assert.assertEquals("apple", subtractSum(10))
    }
}

fun subtractSum(n : Int) : String {
    val subtract = n - n.toString().sumOf {
        it - '0'
    }
    println()
    return if (subtract > 100) subtractSum(subtract)
    else when (subtract) {
